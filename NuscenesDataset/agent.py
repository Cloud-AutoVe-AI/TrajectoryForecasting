...
class Agent:
    ...
    def __init__(self, type=None, attribute=None, track_id=None, agent_id=None, obs_len=None, pred_len=None):
        ...
        self.yaw_global = np.full(shape=(obs_len+pred_len, 1), fill_value=np.nan) # update, 220103
    ...
    def trajectory_preprocessing(self):
        ...
        if (True in np.isnan(self.trajectory[:, 0]).tolist()):
            ...
            # debug ---
            # plt.plot(trajectory_lin_forward[:, 2], trajectory_lin_forward[:, 1], 'bs')
            # plt.plot(self.trajectory[:, 2], self.trajectory[:, 1], 'ro')
            # plt.show()
            # debug ---
