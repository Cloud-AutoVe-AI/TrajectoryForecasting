...
def l2_loss_valid_pos(pred_traj, pred_traj_gt, valid_pos):

    seq_len, batch, _ = pred_traj.size()
    error = (pred_traj_gt - pred_traj)**2
    loss = torch.zeros(1).to(error)

    num_valid_pos = 0
    for a in range(batch):
        ids = valid_pos[:, a, 0]
        num_valid_pos += np.sum(np.array(ids).astype('float'))
        loss += torch.sum(error[ids, a, :])

    return loss / num_valid_pos
